...
class UserDataLogicTest {

    @Test
    fun testUserDataLogic() {
        println("\n=== АНАЛИЗ ЛОГИКИ С РЕАЛЬНЫМИ ДАННЫМИ ПОЛЬЗОВАТЕЛЯ ===")
        
        // Создаем тестовые данные из XML пользователя
        val lipetor = createTestMedicine(
            id = 1754381301015,
            name = "Липетор",
            dosage = "20",
            time = "17:41",
        ...
        val fubuxusat = createTestMedicine(
            id = 1754381353482,
            name = "Фубуксусат",
            dosage = "Полтоблетки",
            time = "16:15",
        ...
        // Проверяем логику группировки (как в DosageCalculator.shouldTakeMedicineInGroup)
        ...
        // Проверяем логику для каждого лекарства (как в shouldTakeMedicineInGroup)
        println("\n--- ЛИПЕТОР (порядок 1) ---")
        val lipetorGroupDay = (daysSinceGroupStart % 2).toInt()
        val lipetorShouldTake = when {
            lipetor.groupOrder <= 0 -> false
            lipetor.groupOrder == 1 -> lipetorGroupDay == 0
            lipetor.groupOrder == 2 -> lipetorGroupDay == 1
            else -> false
        }
        println("День группы: $lipetorGroupDay")
        println("Должен принимать (порядок 1, день 0): $lipetorShouldTake")
        
        // Проверяем, был ли прием вчера (как в shouldTakeMedicineInGroup)
        val yesterday = today.minusDays(1)
        val lipetorWasTakenYesterday = if (lipetor.lastTakenTime > 0) {
            val lastTakenDate = Instant.ofEpochMilli(lipetor.lastTakenTime)
        ...
        val fubuxusatShouldTake = when {
            fubuxusat.groupOrder <= 0 -> false
            fubuxusat.groupOrder == 1 -> fubuxusatGroupDay == 0
            fubuxusat.groupOrder == 2 -> fubuxusatGroupDay == 1
            else -> false
        }
        println("День группы: $fubuxusatGroupDay")
        println("Должен принимать (порядок 2, день 1): $fubuxusatShouldTake")
        
        val fubuxusatWasTakenYesterday = if (fubuxusat.lastTakenTime > 0) {
            val lastTakenDate = Instant.ofEpochMilli(fubuxusat.lastTakenTime)
                .atZone(ZoneId.systemDefault())
                .toLocalDate()
            lastTakenDate == yesterday
        } else {
        ...
        // Финальная логика (как в shouldTakeMedicineInGroup)
        println("\n=== ФИНАЛЬНАЯ ЛОГИКА ===")
        ...
        // Проверяем логику фильтрации (как в getActiveMedicinesForDate)
        println("\n=== ПРОВЕРКА ФИЛЬТРАЦИИ (как в getActiveMedicinesForDate) ===")
        
        val activeMedicines = medicines.filter { medicine ->
            val isActive = medicine.isActive
            val shouldTake = if (medicine.groupId != null) {
                // Логика группировки (упрощенная версия)
                val groupStart = Instant.ofEpochMilli(medicine.groupStartDate)
                    .atZone(ZoneId.systemDefault())
                    .toLocalDate()
                val daysSinceStart = ChronoUnit.DAYS.between(groupStart, today)
                ...
                // Обычная логика (упрощенная)
                val startDate = Instant.ofEpochMilli(medicine.startDate)
                    .atZone(ZoneId.systemDefault())
                    .toLocalDate()
        ...
        println("Лекарств для сегодня (не принятых): ${medicinesForToday.size}")
    ...
    // Модель лекарства для тестирования (упрощенная версия)
