...
# -*- coding: utf-8 -*-
"""
Скрипт для сборки Android проекта с очисткой кэша
"""
...
def clean_gradle_cache():
    """Очищает поврежденный кэш Gradle"""
    print_step(1, "Очистка кэша Gradle")
    
    try:
        # Удаляем только поврежденные файлы кэша
        cache_path = Path('C:/gradle_home/caches')
        if cache_path.exists():
            # Удаляем только journal и metadata файлы
            journal_path = cache_path / 'journal-1'
            if journal_path.exists():
                print_info("Удаление поврежденных journal файлов...")
                import shutil
                shutil.rmtree(journal_path, ignore_errors=True)
                print_success("Journal файлы удалены")
            
            # Удаляем metadata файлы
            modules_path = cache_path / 'modules-2' / 'metadata-2.106'
            if modules_path.exists():
                print_info("Удаление поврежденных metadata файлов...")
                shutil.rmtree(modules_path, ignore_errors=True)
                print_success("Metadata файлы удалены")
        
        print_success("Кэш очищен")
        return True
    except Exception as e:
        print_error(f"Ошибка при очистке кэша: {e}")
        return False
...
def main():
    print("🚀 Сборка Android проекта MedicalNotes (с очисткой)")
    print(f"Время начала: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    
    if not clean_gradle_cache():
        print_error("Очистка кэша не удалась")
        return False
